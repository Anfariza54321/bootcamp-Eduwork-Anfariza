    ...
    <div class="container mx-auto px-4 py-12">
        ...
        <div class="bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row border border-gray-100">
            <div class="md:w-1/2 bg-gray-50 flex items-center justify-center p-8">
                <img src="{{ asset('img/' . $product['namaProduk'] . '.png') }}" alt="{{ $product['namaProduk'] }}"
            ...
            <div class="md:w-1/2 p-8 md:p-12 flex flex-col justify-center">
                <span class="text-blue-600 font-semibold tracking-widest text-sm uppercase mb-2">Original
                    Collections</span>
                <h1 class="text-4xl font-extrabold text-gray-900 mb-4">{{ $product['namaProduk'] }}</h1>

                <div class="flex items-center mb-6">
                    <span class="text-3xl font-bold text-gray-900 mr-4">${{ $product['hargaDiskon'] }}</span>
                    <span class="text-xl text-red-500 line-through">${{ $product['hargaAsli'] }}</span>
                ...
                <p class="text-gray-600 leading-relaxed mb-8">
                    Nikmati kenyamanan maksimal dengan **{{ $product['namaProduk'] }}**. Dibuat dengan material
                ...
                <form action="{{ route('carts.add', $product['id']) }}" method="POST">
                    @csrf
                    <input type="hidden" name="name" value="{{ $product['namaProduk'] }}">
                    <input type="hidden" name="price" value="{{ $product['hargaDiskon'] }}">
