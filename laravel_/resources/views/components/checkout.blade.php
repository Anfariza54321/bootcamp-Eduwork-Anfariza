                    ...
                    @if (count($cart) > 0)
                        @foreach ($cart as $id => $item)
                            ...
                            <article
                                ...
                                <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                                    <div>
                                        <h2 class="text-lg font-semibold text-cyan-300">{{ $item['name'] }}</h2>
                                        <p class="text-sm text-cyan-100/80">Qty: {{ $item['quantity'] }} • Harga: Rp
                                            {{ number_format($item['price'], 0, ',', '.') }}</p>
                                    </div>
                                    <span class="text-sm font-semibold text-cyan-200">Subtotal: Rp
                                        {{ number_format($itemSub, 0, ',', '.') }}</span>
                                </div>
                            </article>
                        @endforeach
                        ...
                        <div class="rounded-xl border border-purple-400 bg-purple-900/20 p-4">
                            <p class="flex justify-between font-semibold text-cyan-100">
                                ...
                                <span>Rp {{ number_format($checkoutTotal, 0, ',', '.') }}</span>
                            </p>
                        </div>
                    @else
                        <div
